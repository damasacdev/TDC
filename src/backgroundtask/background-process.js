    ...
    try {
        ...
        ipcRenderer.on('call-backgroud-process', (event, arg) => {
            try {
                ...
                even_delay = dataConfig[28].value;
        ...
        if (checkSyncQ == false) {
            ...
            for (var j = 0; j < data43File.length; j++) {
                ...
                if (dataConfigSql['dbtypes'] == '0') { //mysql
                    ...
                    countSqlT = "SELECT  COUNT(*) AS dataT FROM `" + dataConstant[0].value + "`.`buffe_transfer` WHERE SUBSTRING_INDEX(`table`,'f_',-1) = SUBSTRING_INDEX('" + data43File[j]['f43'] + "','f_',-1) AND `status` <> 2";
                } else if (dataConfigSql['dbtypes'] == '1') { //mssql
                    countSqlQ = "SELECT  COUNT(*) AS dataQ FROM [" + dataConfigSql['schemas'] + "].[buffe_fetch_queue]   WHERE f_tname = '" + data43File[j]['f43'] + "'";
                    countSqlT = "SELECT  COUNT(*) AS dataT FROM [" + dataConfigSql['schemas'] + "].[buffe_transfer]   WHERE [table] = '" + data43File[j]['f43'] + "' AND [status] <> 2";
                } else if (dataConfigSql['dbtypes'] == '2') { //
...
function getCountTranfer() {
    ...
    if (dataConfigSql['dbtypes'] == '0') { //mysql
        ...
        countSql = "SELECT  COUNT(*) AS countAll FROM `" + dataConstant[0].value + "`.`buffe_transfer` WHERE `status` <> 2";

    } else if (dataConfigSql['dbtypes'] == '1') { //mssql

        countSql = "SELECT  COUNT(*) AS countAll FROM [" + dataConfigSql['schemas'] + "].[buffe_transfer] WHERE [status] <> 2 ";
